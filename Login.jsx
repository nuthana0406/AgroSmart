...
export default function Login() {
  ...
  const dest = returnTo === "/" ? "/home" : returnTo;
    ...
    <AuthLayout
      ...
      footer={
        <>
          {t("dontHaveAccount")}{" "}
          <Link
            to={"/register" + (dest !== "/home" ? "?returnTo=" + encodeURIComponent(dest) : "")}
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="space-y-2">
          <Label htmlFor="email">{t("mobileOrEmail")}</Label>
        ...
        <Button type="submit" className="w-full h-12 font-medium bg-primary text-primary-foreground hover:bg-primary/90" disabled={loading}>
          {loading ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> {t("loggingIn")}</> : t("loginBtn")}
        </Button>
      </form>
    </AuthLayout>
  );
